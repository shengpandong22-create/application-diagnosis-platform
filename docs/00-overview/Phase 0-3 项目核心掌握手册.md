...
## 7. 链路四：真实 Java 故障怎样进入 Python Agent
...
## 9. Trace、Report、Audit 和 Plan 不要混淆
...
## 10. 十二个关键文件地图：知道位置，不等于全部通读

这一节用于建立代码导航能力，不要求把十二个文件逐行读完。真正强制抽查的源码已经在第 24 节压缩为六个锚点方法；下表其余内容主要用于面试追问或后续修改时快速定位。
...
> 我实现了一个面向应用故障诊断的本地 Agent 平台。它可以基于真实 Java 服务产生的日志，受控读取源码、配置、知识和健康状态，通过多轮 Tool Calling 形成诊断结论。项目的重点不是单纯接大模型，而是用状态机、工具白名单、预算、Evidence 引用校验、Trace 和人工确认，把概率性推理约束在可追踪的工程闭环中。
...
7. 边界：单机参考实现，尚无分布式采集、Worker 和自动修复。
...
当前还是单机原型：运行并发保护不跨进程、服务目录不自动发现、真实模型评测规模有限、ApplicationService 仍有基础设施依赖。下一步应优先补服务历史视图、评测数据集与异步运行边界，而不是盲目增加 Agent 数量。
...
### 17.2 AgentTerminationReason 与 DiagnosisStatus 不是一回事
...
### 18.4 为什么没有一次大事务包住整个 AgentRun

LLM 和工具调用可能持续较长时间。持有数据库事务会增加锁等待、连接占用和失败回滚范围。当前设计按事实逐步持久化 AgentRun、ToolRun 和 Evidence，使失败时已经发生的动作仍可追踪。代价是需要明确中间状态和幂等规则。

### 18.5 当前 Repository 边界的真实评价

领域层基本不依赖 FastAPI、SQLAlchemy 和模型供应商；但 ApplicationService 直接创建部分 SQLAlchemy Repository，并持有 session factory。这是为了 Phase 0～3 在单机项目中控制复杂度的务实妥协。
